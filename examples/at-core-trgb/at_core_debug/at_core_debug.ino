...
struct StatusData {
    int mode,gps_sat,csq,frames,alt,spd,hdg,bat;
    float lat,lon;
    bool gps_fix,sd_ok,flarm_ok,adsb_ok,valid;
};
struct FlightData { float gforce_z; int co_ppm,rpm,phase; bool valid; };
#define MAX_TRF 5
struct TrafficEntry { char cs[9]; int dist_m,alt_m,bearing,hdg,type; bool visible; };
...
static lv_obj_t *r_title,*r_mode,*r_gps,*r_lte,*r_sd,*r_ble,*r_flarm,*r_adsb,*r_coords,*r_bat_s1;
static lv_obj_t *r_phase,*r_alt,*r_spd,*r_hdg,*r_gforce,*r_co,*r_rpm;
#define RADAR_CX   240
#define RADAR_CY   240
#define RADAR_R_PX 200.0f
#define RADAR_MAX_M 20000.0f
static lv_obj_t *r_trf_img[MAX_TRF];
static lv_obj_t *r_trf_vect[MAX_TRF],*r_trf_lbl[MAX_TRF];
...
void parseTraffic(const char* j){JsonDocument d;if(deserializeJson(d,j))return;
    ...
    for(int i=0;i<g_traffic.count;i++){
        ...
        g_traffic.t[i].bearing=d["t"][i]["b"]|0;g_traffic.t[i].hdg=d["t"][i]["c"]|0;
...
void buildPage1(){
    ...
    mkKV(p,285,"BAT",&r_bat_s1,"---%",C_GREY);
...
static lv_obj_t* mkRing(lv_obj_t* p, int r){
    lv_obj_t* o=lv_obj_create(p);lv_obj_set_size(o,r*2,r*2);
    lv_obj_set_pos(o,RADAR_CX-r,RADAR_CY-r);
    lv_obj_set_style_bg_opa(o,LV_OPA_TRANSP,0);
    lv_obj_set_style_border_color(o,C_GREY,0);
    lv_obj_set_style_border_width(o,1,0);
    lv_obj_set_style_radius(o,LV_RADIUS_CIRCLE,0);
    lv_obj_set_style_pad_all(o,0,0);
    lv_obj_clear_flag(o,LV_OBJ_FLAG_SCROLLABLE);return o;}
static lv_obj_t* mkRadarLine(lv_obj_t* p, lv_point_t* pts, lv_color_t col){
    ...
    lv_obj_add_flag(l,LV_OBJ_FLAG_HIDDEN);return l;}

void buildPage3(){
    lv_obj_t* p=g_pages[2];
    mkLbl(p,"TRAFFIC",C_BLUE,&lv_font_montserrat_20,LV_ALIGN_TOP_MID,0,30);
    // Range rings + labels
    mkRing(p,50); mkRing(p,100); mkRing(p,200);
    mkLblP(p,"5km", C_GREY,&lv_font_montserrat_14,RADAR_CX+4,RADAR_CY-50+3);
    mkLblP(p,"10km",C_GREY,&lv_font_montserrat_14,RADAR_CX+4,RADAR_CY-100+3);
    mkLblP(p,"20km",C_GREY,&lv_font_montserrat_14,RADAR_CX+4,RADAR_CY-200+3);
    // Own-ship dot
    lv_obj_t* own=lv_obj_create(p);lv_obj_set_size(own,6,6);
    lv_obj_set_pos(own,RADAR_CX-3,RADAR_CY-3);
    lv_obj_set_style_bg_color(own,C_WHITE,0);
    lv_obj_set_style_radius(own,LV_RADIUS_CIRCLE,0);
    lv_obj_set_style_border_width(own,0,0);
    lv_obj_clear_flag(own,LV_OBJ_FLAG_SCROLLABLE);
    // No-traffic label
    r_noTraffic=mkLbl(p,"No traffic",C_GREY,&lv_font_montserrat_16,LV_ALIGN_CENTER,0,60);
    // Aircraft slots — init points at center (zero-length, invisible)
    for(int i=0;i<MAX_TRF;i++){
        ...
        lv_obj_set_style_text_color(r_trf_lbl[i],C_WHITE,0);
        lv_obj_set_style_text_font(r_trf_lbl[i],&lv_font_montserrat_14,0);
        lv_obj_add_flag(r_trf_lbl[i],LV_OBJ_FLAG_HIDDEN);}
...
void updateAllPages(){
    ...
    if(g_status.valid){
        ...
        if(g_status.bat<0){lv_label_set_text(r_bat_s1,"---%");lv_obj_set_style_text_color(r_bat_s1,C_GREY,0);}
        else{snprintf(b,32,"%d%%",g_status.bat);lv_label_set_text(r_bat_s1,b);
        lv_obj_set_style_text_color(r_bat_s1,g_status.bat>=50?C_GREEN:g_status.bat>=20?C_AMBER:C_RED,0);}
    ...
    // P3 — radar
    if(g_traffic.valid){
        g_traffic.count>0?lv_obj_add_flag(r_noTraffic,LV_OBJ_FLAG_HIDDEN):lv_obj_clear_flag(r_noTraffic,LV_OBJ_FLAG_HIDDEN);
        for(int i=0;i<MAX_TRF;i++){
            if(i<g_traffic.count){
                TrafficEntry&e=g_traffic.t[i];
                // position on radar
                float dp=e.dist_m*(RADAR_R_PX/RADAR_MAX_M);if(dp>RADAR_R_PX)dp=RADAR_R_PX;
                float br=e.bearing*(float)M_PI/180.0f;
                int sx=RADAR_CX+(int)(sinf(br)*dp);
                int sy=RADAR_CY-(int)(cosf(br)*dp);
                ...
                r_vect_pts[i][0]={(lv_coord_t)(sx+(int)(12.f*sinf(br))),(lv_coord_t)(sy-(int)(12.f*cosf(br)))};
                r_vect_pts[i][1]={(lv_coord_t)(sx+(int)(27.f*sinf(br))),(lv_coord_t)(sy-(int)(27.f*cosf(br)))};
                lv_line_set_points(r_trf_vect[i],r_vect_pts[i],2);
                lv_obj_clear_flag(r_trf_vect[i],LV_OBJ_FLAG_HIDDEN);
                // callsign
                lv_label_set_text(r_trf_lbl[i],e.cs);
                lv_obj_set_style_text_color(r_trf_lbl[i],e.visible?C_WHITE:C_AMBER,0);
                lv_obj_set_pos(r_trf_lbl[i],sx-20,sy-24);
                lv_obj_clear_flag(r_trf_lbl[i],LV_OBJ_FLAG_HIDDEN);
            }else{
                lv_obj_add_flag(r_trf_img[i],LV_OBJ_FLAG_HIDDEN);
