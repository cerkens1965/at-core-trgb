...
#include <esp_heap_caps.h>   // heap_caps_malloc_extmem_enable() — force le handle I2S en RAM interne
                             // (cf audioInit : gdma ISR IRAM-safe exige un user-context interne)
...
#ifdef PANEL_WS241
#define RAD_CY 225          // WS-241 = dalle 600×450 → centre vertical 225
...
// ⚠️ AUDIO MUET (v85) — connu, NON corrigé. i2s_channel_init_std_mode échoue au boot :
// CONFIG_GDMA_ISR_IRAM_SAFE=1 → le callback DMA du canal tourne en ISR IRAM-safe et gdma EXIGE que
// son user-context (= le handle du canal, passé à gdma_register_tx_event_callbacks) soit en RAM
// INTERNE. Or le driver I2S alloue le handle en MALLOC_CAP_DEFAULT → PSRAM sur ce build →
// "user context not in internal RAM" → init KO (non-fatal, le boot continue, g_aud_ok reste false).
// PAS corrigeable ici : GDMA/I2S_ISR_IRAM_SAFE sont figés dans les libs précompilées pioarduino.
// heap_caps_malloc_extmem_enable() ci-dessous n'agit QUE sur malloc()/heap_caps_malloc_default, pas
// sur le heap_caps_malloc(DEFAULT) direct du driver → SANS EFFET (laissé, inoffensif). VRAI fix (à
// faire quand le DAC PCM5102A sera câblé) : driver i2s legacy driver/i2s.h, ou framework recompilé
// avec CONFIG_I2S_ISR_IRAM_SAFE=y. Ce qui reste utile ici : cleanup + log d'erreur (cf bas de fn).
#ifndef CONFIG_SPIRAM_MALLOC_ALWAYSINTERNAL
#define CONFIG_SPIRAM_MALLOC_ALWAYSINTERNAL 4096
#endif
static void audioInit(){
    heap_caps_malloc_extmem_enable(SIZE_MAX);   // (sans effet sur l'alloc du driver — cf note ci-dessus)
    i2s_chan_config_t cc = I2S_CHANNEL_DEFAULT_CONFIG(I2S_NUM_0, I2S_ROLE_MASTER);
    esp_err_t rc = i2s_new_channel(&cc,&g_aud_tx,NULL);
    ...
    if(rc==ESP_OK) rc = i2s_channel_init_std_mode(g_aud_tx,&sc);
    heap_caps_malloc_extmem_enable(CONFIG_SPIRAM_MALLOC_ALWAYSINTERNAL);  // restaure le seuil PSRAM
    if(rc!=ESP_OK){                             // échec → libère le canal, pas de fuite de handle
        if(g_aud_tx){ i2s_del_channel(g_aud_tx); g_aud_tx=nullptr; }
        Serial.printf("[AUD] init I2S KO (%s)\n", esp_err_to_name(rc));
        return;
    }
...
#ifdef BOARD_T4S3
    // WS-241 (dalle 600×450, UI_OY=0) : on descend le cluster bas de +15 (pas +30) pour garder
