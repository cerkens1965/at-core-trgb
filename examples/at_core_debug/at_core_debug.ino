...
static const uint8_t kAlertVolPct[4] ={0,35,60,90};                 // OFF/LOW/MED/HIGH → % amplitude du sinus
...
struct CfgData { uint8_t scale_nm,brightness,trf_src; bool dist_nm,alt_ft,dark,show_grnd,wifi_en,aip_en,ad_heli,spd_kt; int16_t vfilt_ft; uint8_t icon_sz; bool show_cs,show_vdiff; uint8_t circuit_ovr; uint8_t alert_snd; };  // alert_snd (v105) : volume alerte trafic audio 0=OFF 1=LOW 2=MED 3=HIGH (en fin de struct → init agrégat inchangé, chargé par cfgLoad)  // circuit_ovr : 0=AUTO 1=CIRCUIT(forcé) 2=ROUTE(forcé) — override alerte (en fin de struct → init agrégat inchangé, défaut 0)
...
static void audioBeep(int freq,int ms,int volPct){
    ...
    i2s_zero_dma_buffer(I2S_NUM_0);   // coupe la porteuse après le bip (silence franc)
}
...
static void TaskAudioAlert(void*){
    ...
    for(;;){
        if(g_aud_test_req){ g_aud_test_req=false; audioTestChime(); lastLvl=0; lastBeep=millis(); vTaskDelay(pdMS_TO_TICKS(20)); continue; }
...
static void radarZoom(int dir){   // ENCODEUR : ±1 NM par cran (dir>0 = zoom in = portée diminue)
    int nm=(int)g_cfg.scale_nm + ((dir>0)?-1:+1);
    if(nm<1)nm=1; if(nm>40)nm=40;                 // portée radar bornée 1..40 NM
    g_cfg.scale_nm=(uint8_t)nm; cfgSave(); updSetPage();
}
...
void alertEngineTick(){
    ...
    g_aud_alert_level = g_threat.level;   // (v105) publie le niveau pour TaskAudioAlert (0 si pas de menace/au sol)
}
