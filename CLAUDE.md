...
- v299 : TAILLES (Christophe : « textes trop petits ») — page 1 : lignes 70 px (nom sans 28, sous-titre 18, chevron 24), calibrée sur 5 lignes Display→System (92 → 442) ; sur DEV la 6e ligne Diagnostic est atteignable par défilement du menu (SCROLLABLE, scrollbar off). Sections : `SEC_ROWH` 56, libellé sans 24, pas 64 (Display ET Traffic défilent), valeurs mono 20, chevron 22, pilules 52×28 knob 22 (`p1SwitchSet` déduit la géométrie), segments 40 px cellules sans 18 (cw 108 / 90 / 74), slider 200×8. Overlays : `mkActRow` 56 sans 24, `mkInfoRow` sans 20 + mono 18, `rowValue` mono 20, Diagnostic 84/122/160 + actions 204 pas 60 (→ 440), popups mono 20 panneau défilant borné 430, Flight Logs lignes mono 18 + boutons sans 18, WiFi Setup textes 20 + boutons 52. ÉCHELLE DE RÉFÉRENCE ÉCRAN : titre 28 · ligne menu 28/18 · ligne réglage 24 · valeur mono 20 · aide 18 · bouton 18-20 · rien < 13.
- v300 : défilement — focus sur la 1re ligne ⇒ `lv_obj_scroll_to_y(parent,0)` (page 1 via LV_EVENT_FOCUSED sur g_menuBtns[0], sections via erSetState ai<=1). Page 1 flotte : 5 lignes → pas de défilement (pad bas 8 seulement sur DEV). Autres pages : Diagnostic/Updates/Flight Logs/WiFi Setup ne défilent pas (tout tient en 450) ; popup de choix défile au doigt seulement.
- v301 : monogramme AirKi 32 px (`mkHdrMark`, img_airki_mark zoom 85 pivot 0,0) à (20,14) devant les titres (x 64) : page 1, sections (`s_hdr_mark` remonté au-dessus du bandeau), ovHeader, WiFi Setup.
- v302 : FLIGHT LOGS verticale → liste à GAUCHE (20,104 · 340×326, 7 lignes) + colonne de 6 boutons à DROITE (200×48, x 380-580, pas 58 : Send last · Send selected · Send unsent · Delete · Delete all · Cloud upload ON/OFF), WiFi mono 14 à (20,78). v303 : monogramme d'en-tête 44 px (zoom 117) à (20,10), titres à x 76.
- v305 : (1) ICON SIZE au boot : `radarApplyIconSz()` = seul chemin (création radar + cbSegN kind 1) → zoom images kIconZoom + polices libellés WS241 S/M/L = mono 18/13 · 22/14 · 28/18 + triangle ×0,8/1,0/1,3. Avant : polices v284 fixes au boot, kCsFont/kAltFont (sans 22/40) après re-sélection → incohérence perçue comme « petit au boot ». (2) FLIGHT LOGS : 3 commandes — Send (cochés sinon tous les non-envoyés), Delete sent (.up, 2 taps), Cloud upload ; « Delete all » seulement sur DEV. Send last / Send selected / Send unsent retirés de l'UI (callbacks conservés).
- 20/09 soir : **ATV 305 PUBLIÉ FLOTTE (ws241)** sur demande (« pousses en prod ») — Pierre (écran 263) ne parvenait pas à charger le DEV : son boîtier est déjà en 211 (= s3dev, rien à charger) mais son écran 263 lit ws241 ; flotte = ATC 211 / ATV 305.
- INCIDENT 20/09 22:28 : écran WS-241 du banc flashé PAR ERREUR avec le firmware boîtier (ports USB permutés). Reflashé WS-241 305, NVS intact. RÈGLE : `PORT=$(bash ../tools/flash_guard.sh atv)` avant tout upload (série USB A4:CB:8F:D7:71:E8), jamais un nom de port en dur.

- 2026-09-23 : **ÉTAT WIFI SANS AMBIGUÏTÉ (AKview v309)** — retours de vol Christophe. Page **WiFi Setup** : bandeau d'état en haut (pastille + SSID + état + IP), rafraîchi à chaque STATUS ; bouton **Test** ajouté sur la page (il n'existait que dans Diagnostic) avec le RÉSULTAT à l'écran (« testing… » → CONNECTED / NOT FOUND / FAILED / no answer) ; ligne **Settings > System > WiFi Setup** porte le SSID + l'état sans ouvrir la page ; clavier 214 → 180 px. **Une seule formulation** (`wifiStateLine`, utilisée par WiFi Setup, Diagnostic et Flights). ⚠️ RÈGLE : `wst` est **collant** côté AKcore (jamais remis à 0 à la déconnexion) → on n'écrit « CONNECTED » QUE si `wip` (IP) est non vide, car le boîtier ne remplit l'IP que pendant l'association ; sinon « last connection OK, n min ago » (âge horodaté côté écran, `g_wifi_chg_ms`). Aucun changement firmware boîtier (le STATUS est au ras du MTU 509 o). Flashé écran n°2, publié `ws241dev`.

- 2026-09-23 : **PUBLIÉ EN FLOTTE** (« pousse tout en prod ») — AKcore **212 → 217** (canal `s3`) et AKview **305 → 315** (canal `ws241`). Contenu : boîtier 213-215 (origine par réseau WiFi, oubli de réseau, saisie WiFi différée = fin des paniques rst=4), 216 (état sol dès 10 kt + GPS 1 Hz + option bandes basses LTE), 217 (« Cloud upload » respecté en fin de vol, défaut NVS passé à ON) ; écran 306-315 (lockup accueil, libellés AKcore/AKview, GND, textes agrandis, état WiFi sans ambiguïté, icônes 34 px, molette sur appairage + actions de vol, boutons AirKi unifiés). ⚠️ AUCUNE de ces versions n'a volé : elles ont été validées au banc et au sol uniquement. Le canal WROVER (`production`) reste en 212 — flotte 100 % S3 depuis le 15/09.

- 2026-09-23 après-midi : **PUBLIÉ EN FLOTTE** (« pousse la 218 et la 317 en flotte ») — AKcore **217 → 218** (canal `s3`) et AKview **315 → 317** (canal `ws241`). Contenu : boîtier 218 = **RÈGLE N°1, le GPS ne s'arrête jamais** (attentes du chemin d'envoi 3 s/5 s → 400/800 ms, horodatage du dernier fix réel + mesure du plus long trou dans events.log, **fix de plus de 2 s = invalide** donc NMEA en 'V' et ownship GDL90 supprimé, priorité à l'acquisition sur les réouvertures/escalades) ; écran 316-317 = pastille Start/End flight retirée du radar, **clic court molette = Settings**, **appui long 800 ms = radar d'où qu'on soit**, feuille d'actions conservée en appui long TACTILE. ⚠️ Ces deux versions **n'ont pas volé** — validées au banc et au sol seulement. Le canal WROVER (`production`) reste en 212.
